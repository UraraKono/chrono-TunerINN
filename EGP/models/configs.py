...
class MPCConfigEXT:
    ...
    Rk: list = field(
        default_factory=lambda: np.diag([0.0000015, 2.0])
    ...
    Qk: list = field(
        default_factory=lambda: np.diag([13.5, 13.5, 4*13.5, 0.0, 0.0, 0.0, 0.0])
    ...
    dlk: float = 1.0  # dist step [m] kinematic
...
class MPCConfigGPFrenet:
    ...
    TK: int = 15
    Rk: list = field(
        default_factory=lambda: np.diag([0.0000008, 2.0]) # 0.0000008, 2.0
    )  # input cost matrix, penalty for inputs - [accel, steering_speed]
    Rdk: list = field(
        default_factory=lambda: np.diag([0.0000008, 2.0]) # 0.0000008, 2.0
    )  # input difference cost matrix, penalty for change of inputs - [accel, steering_speed]
    Qk: list = field(
        default_factory=lambda: np.diag([5.5, 10.5, 4*13.5, 0.0, 0.0, 0.0, 0.0])
        # [5.5, 10.5, 5.0, 0.0, 0.0, 0.0, 0.0]
        # [13.5, 13.5, 5.5, 13.0, 0.0, 0.0, 0.0]
    )  # state error cost matrix, for the next (T) prediction time steps
    Qfk: list = field(
        default_factory=lambda: np.diag([5.5, 10.5, 4*13.5, 0.0, 0.0, 0.0, 0.0])
