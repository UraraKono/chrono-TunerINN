...
from .closest_point import get_closest_point_vectorized
from .console_handling import color_map
...
class Track:
    ...
    def frenet_to_cartesian(self, pose):
        ...
        # print(diff)
        if len(diff[diff <= 0]) == 0:
            print(f"Error: s = {pose[0]} is out of range of the track in frenet_to_cartesian")
            segment_id = 0
        else:
            segment_id = np.argmax(diff[diff <= 0])
        # segment_id = np.argmax(diff[diff <= 0])  # should be always id of the point that has smaller s than the point
