...
class MPPI(MPC):
    """An MPPI based planner."""
    def __init__(self, config, n_iterations=5, n_steps=16, n_samples=16, temperature=0.01,
                damping=0.001, a_noise=0.5, scan=True,
    ...
    def update(self, mpc_state, env, env_state, rng, da):
        ...
        def iteration_step(input_, _):
            ...
            r_sample, s, mb_dyna = jax.vmap(self.rollout, in_axes=(0, None, None))(
                a, env, env_state
            )
            ...
            # s_np = np.asarray(jax.device_get(s))[ind]
            # axs.plot(env.reference[1:, 0] - env.reference[0, 0], env.reference[1:, 1] - env.reference[0, 1], '.')
            # axs.plot(s_np[:, 0] - env.reference[0, 0], s_np[:, 1] - env.reference[0, 1], '.')
            # axs.axis('equal')
            # print('min', r[ind])
            # plt.show()
            ...
            # r = r + jnp.sum(jnp.power(a, 2.0), axis=2)
            ...
            # heuristic for regaining control
            if self.mode == 'nf' or self.mode == 'mb':
                exploration = jnp.sum(jnp.var(s[:, -1, :2], axis=0))
                if exploration < 0.5:
                    a_opt = a_opt.at[0].set(jnp.array([jnp.sign(env_state[2]) * -(1-2*exploration), -(1-2*exploration)]))
            ...
            # a_opt = a[min_ind]
            # s_opt = s[0]
            return (a_opt, a_cov, s, s_opt, rng, r_sample, a, mb_dyna), None
        
        predicted_states = []
        if not self.scan:
            for _ in range(self.n_iterations):
                (a_opt, a_cov, s, s_opt, rng, r_sample, a, mb_dyna), _ = iteration_step((a_opt, a_cov, rng, da), None)
                predicted_states.append(s)
        else:
            (a_opt, a_cov, rng), _ = jax.lax.scan(
                iteration_step, (a_opt, a_cov, rng), None, length=self.n_iterations
            )
        return (a_opt, a_cov), jnp.stack(predicted_states), s_opt, r_sample, a, mb_dyna
    ...
    def rollout(self, actions, env, env_state):
        ...
        if not self.scan:
            ...
            s, r, mb_dyna = jax.tree_util.tree_map(lambda *x: jnp.stack(x), *scan_output)
        else:
